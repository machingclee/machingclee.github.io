...
This issue is especially easy to overlook when routes are generated programmatically — for instance, by reading filenames from a directory or slugs from a database. In those cases the generator receives a raw string that may contain non-ASCII characters, and concatenating it directly into a \`<loc>\` tag silently produces an invalid URL. Encoding must be applied explicitly during generation, before the URL is written into the XML.

\`\`\`xml
<loc>https://machingclee.github.io/blog/article/Live2D-%E5%BF%83%E5%BE%97</loc>
\`\`\`

Leaving Chinese characters unencoded causes GSC to flag those entries as **網址無效** (Invalid URL) and skip them during indexing.
