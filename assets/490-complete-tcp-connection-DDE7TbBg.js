...
####  Uncontrolled connection timeout 

- We have mentioned earlier that we have to loop though all possible values in \`ip_local_port_range\` and pick an available one. The time taken for this process is very short when we have sufficient ports that are not in use, however, this can take very long when there are not sufficient ports. Let's delve into detail.
...
- Within each loop we need to wait for \`unlock\` and do computational searches from the hash table. Note that the lock here is a spin lock, it never puts the process to sleep, and will try to indefintely exhause the CPU resource to try getting the lock.

- Suppose the \`ip_local_port_range\` is set to 10000~30000, and they are used up, then each connection will need to loop for about 20000 times until it quit the loop. At that time, large amount of search in hash table and waiting for spin-lock happen, leading to the huge computational cost.

- They are many ways to get around this problem. Increase the range of \`net.ipv4.ip_local_port_range\`, chagne to use long polling, or recycle the \`TIME_WAIT\` as soon as possible.

#### The Packet Lost in the First Handshake`;export{e as default};
