...
@Component
class LogAspect {
    @Around("@annotation(com.billie.payment.annotation.LogParam)")
...
Sometimes we would like to have interceptor applied to all of the methods, we replace `@Target(AnnotationTarget.FUNCTION)` by `@Target(AnnotationTarget.CLASS)` and write:

```kotlin 
@Aspect
@Component
class LogAspect {
    @Around("@within(com.billie.payment.annotation.LogParam)")
    fun around(point: ProceedingJoinPoint): Any? {
        try {
            val signature = point.signature as MethodSignature
            val method = signature.method
