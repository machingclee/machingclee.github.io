...
In fact we just need the header file in `WinReg/WinReg.hpp`, we can create a CMakeProject and simply include this header file.

Now in order to register the application to a url scheme, we use the following:
...
- ```cpp
  ...
  means that at the folder `HKEY_CURRENT_USER`, we create a nested folder sturcture as instructed.

- `key.SetStringValue(std::string key, std::wstring value)` is the same as setting key-value pair in a registry folder.

  For example, what `key.SetStringValue(L"URL Protocol", L"")` does is to create a new key-value pair at the current working directory (which is `Software\\Classes\\eyecatcher`):
