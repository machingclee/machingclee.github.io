...
\end{aligned}
$$
where $\texttt{pos} \in \mathbb N$ and $i = 0, 1, 2,\dots, d_\text{model}-1$.
...
- those from "furture word" in self-attention module of decoder.
...
d_\text{model}//\texttt{heads}  & \text{for }\texttt{MultiHeadAttention}.
\end{cases}
$$
...
$$
The matrix $\texttt{attn}$ can be decomposed as $A\times 0$ for some $A\in \mathbb R^{q\times k_1}$ and $0\in \mathbb R^{q\times k_2}$, where $k_1+k_2=d_k$. Our attention will be the linear span of values (the rows) in $V\in \mathbb R^{k\times d_k}$:
$$
\begin{aligned}
\texttt{context} 
& = \texttt{attn} \times V\\
...
\end{bmatrix}
\right)^T \\
&\in \mathbb R^{q\times d_k}
\end{aligned}
$$
The last few rows of $V$ ($v_i$ for $i\ge k_2$) has no contribution to the attention (few $=$ number of paddings). 
