...
3. 這位 Tech Lead 沒有使用 ***middleware*** 的經驗，所以不理解這個字是甚麼（那個舊 Spring Boot project 也應該是出自其他同事的，恐怕他也不知道甚麼是 AOP）。

    在***寫後端***這個 context 下，跟我爭論 middleware 這個字可以有***很多***意思。可是有後端知識的人，但凡寫過 C#，寫過 nodejs，寫過 golang，都不會覺得 middleware 這個字有任何歧義。

4. 這個舊 Spring Boot Project 有一個絕妙的點，整個項目 ***完全沒有*** repository。你都使用 mongo 了，不是有 `spring-data-mongodb` 可以用嗎？難道所有簡單的 "query" 都要自己手擼出來嗎。
...
#### 得來不易的機會
##### 我獨自升級 (2023 年還沒有 Vibe Coding) 

從後端，Schema Design，上 Cloud，走 Lambda，CI/CD，一手包辦。
...
你在哪一間公司在 "有一位比你更 senior 的人存在" 下可以隨便做決策的？Senior 很大機會把最有價值的工作搶來做的。

正好這位 Tech Lead 的離去，令我有隨意發揮的機會。同時我的發揮令到公司的原型產品能如期推出。我敢肯定這位 Tech Lead 繼續留下來的話，原型開發進度不可能有那麼快。
...
3. 所有的 cloud infrastructure。以及後來使用 infrastructure as code via Terraform 來達成 infrastructure 的可重覆性。
...
1. Database 交互主要用一種名為 Kysely 的 query builder。儘管 architecture 走的是 controller service，但沒有 repository。
...
等等。它們從根本上沒法避免。你可以告訴我同樣的 domain logic
- `memberService.joinProject` 跟 
- `projectService.addMember` 

哪個是正確的？**沒有對錯**，因為後端從根本沒有 Aggregate 這個概念，所有 resource 的建立都沒有層級去規範。你可以想像跟 Project 有關的 domain logic 四處亂跑了嗎？

說到底，怎樣才能構成一個 Service，他的建立是基於甚麼原則？如果沒有原則，那就是即興，亂源而已。

###### 學習系統設計的方法論（思想框架，實行方法）
...
今年 7 月底大陸同事被辭退， 8 月底一位前端同事離職，因為他找到了一份銀行（mox bank）的前端工作；此外，現在公司一年多沒有薪金調整。種種的原因也促使我嘗試找找更好的機會。
...
展示這些 "作品" 後面試機會還挺多的。我的方向是全端 / 純後端，技術棧方面後端找的是 Spring Boot, Nodejs 或者是 Rust，前端找的是 React / React-Native，且***拒絕所有***純前端的工作，因為這與我的職涯規劃相沖。
...
- 一份令人容易閱讀的 CV，這吃一點點美術。你不需要有視角誘導，但你要有能力突出重點。這包括︰
  - 資訊量控制（虛實，在排版中就是留白，字的疏密度）；用心的排版

    ![](/assets/img/2025-12-04-11-35-49.png?width=300px)
    

  - 講重點，不要加上 "我令到系統少了 85% error，前端提昇了 30% 速度"。這些 "沒辦法證明"，"面試問不到" 的東西遠比想像中虛。如實說出你幹過甚麼就好。
...
另一邊廂，面試中國銀行後，過了兩個禮拜我才收到中國銀行對我有興趣的消息，並從獵頭那邊知道他們想給我 offer。再 3 個禮拜後才正式通知我可以去簽約 ...（太多不確定性了，我完全不知道中國銀行對我的 case 進行到哪一個地步），整個過程非常的煎熬。
