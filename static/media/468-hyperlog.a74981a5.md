...
$$\displaystyle P(\text{leading zeros} = k) = \left(\frac{1}{2}\right)^{k+1}$$

#### The Core Intuition

When you hash an element, the output is a binary string where each bit behaves like an independent fair coin flip. HyperLogLog counts the **position of the leftmost 1-bit** (i.e., the number of leading zeros + 1) in this hash.
...
If you observe a maximum of $k$ leading zeros across all hashed elements, the probability of that happening by chance is $\left(\frac{1}{2}\right)^k$. This means you are likely to be needed to hash roughly $2^k$ distinct elements before hitting such a rare outcome.
...
Each element has a $\left(\frac{1}{2}\right)^k$ probability of producing $k$ leading zeros. So the expected number of elements you need to hash before seeing $k$ leading zeros for the first time is $2^k$.

Conversely, after hashing $n$ distinct elements, the **maximum** leading zeros $R$ you have observed is expected to satisfy:

$$2^R \approx n \quad \Longrightarrow \quad R \approx \log_2 n$$

For example, with $n = 1{,}000{,}000$ distinct users:

$$R \approx \log_2(1{,}000{,}000) \approx 19.93 \approx 20$$

So you'd expect the maximum leading zeros across all hashed elements to be around 20. Plugging back in:

$$\hat{n} \approx 2^{20} = 1{,}048{,}576$$
...
- You have $n = 1{,}000{,}000$ distinct elements
- By pure bad luck, the **very first** element you hash produces 30 leading zeros
- Your estimate: $\hat{n} \approx 2^{30} = 1{,}073{,}741{,}824$ — a **1,000x overestimate** from one unlucky hash

The solution is to not rely on a single global max. Instead, collect many **independent** estimates of $R$ and average them — outliers cancel out.

**Approach 1: Multiple independent hash functions** (used in older algorithms like Flajolet-Martin, 1985)

Run $m$ independent hash functions on each element, producing $m$ independent values of $R_1, R_2, \ldots, R_m$. Average them. The downside: you must compute $m$ hash functions per element, which is expensive.

**Approach 2: Bit splitting** (used in HyperLogLog, Flajolet et al. 2007)

Use a **single** hash function, but split its output into two parts:
...
The final estimate combines all 4 registers via harmonic mean — instead of one unreliable global max, you now have 4 independent estimates averaged together.
...
**Why call them "registers"?**

Each slot is called a **register** — borrowing the term from CPU registers (small, fast memory cells storing a single value). A register here is just a 6-bit integer (values 0–63) that stores the **maximum leading zeros seen so far** for every element routed to it.

With $m = 2^b$ registers, the variance from any single outlier is diluted across all $m$ independent estimates. HyperLogLog combines them using a **harmonic mean** (chosen because it is less sensitive to outliers than arithmetic mean):

$$\displaystyle \hat{n} = \alpha_m \cdot m^2 \cdot \left( \sum_{j=1}^{m} 2^{-M_j} \right)^{-1}$$

where $M_j$ is the max leading zeros in register $j$, and $\alpha_m$ is a bias correction constant.

**Why $m^2$ when there are only $m$ summands?** The two factors of $m$ come from different places:

- Each register $j$ sees roughly $n/m$ elements (spread evenly). Its leading-zero count estimates the **local** count:

  $$2^{M_j} \approx \frac{n}{m}$$

- In slot (register) $j$ we approximately estimate the global cardinality by $\hat{n}_j$:

  $$\hat{n}_j = m \cdot 2^{M_j}$$

- Take the harmonic mean of all $m$ per-register estimates $\hat{n}_j = m \cdot 2^{M_j}$:

  $$\hat{n} = \frac{m}{\displaystyle\sum_{j=1}^{m} \frac{1}{m \cdot 2^{M_j}}} = \frac{m}{\dfrac{1}{m}\displaystyle\sum_{j=1}^{m} 2^{-M_j}} = \frac{m^2}{\displaystyle\sum_{j=1}^{m} 2^{-M_j}}$$
...
$$\displaystyle \alpha_m = \left( m \int_0^\infty \left( \log_2 \frac{2+u}{1+u} \right)^m du \right)^{-1}$$
...
$$\displaystyle \alpha_{16384} \approx \frac{0.7213}{1 + 1.079/16384} \approx 0.7213$$

$\alpha_m$ is solved once offline — Redis simply embeds `0.7213` as a hardcoded constant in its C source code. This factor is introduced from [PEOF] listed in [#ref].

Redis uses $b = 14$, giving $m = 16384$ registers × 6 bits each = **12 KB** of memory, with a standard error of only **~0.81%** — regardless of whether you have 1,000 or 1,000,000,000 unique elements.
...
**Why not SHA256 or MD5?** They would work mathematically, but are far slower. HyperLogLog is called millions of times — cryptographic strength is unnecessary. MurmurHash is ~10x faster while providing sufficient uniformity.

**Why not a simple `x % N`?** Modulo operations cluster outputs and destroy the uniform distribution — e.g., all even inputs map to even outputs, so bits are no longer independent coin flips.
