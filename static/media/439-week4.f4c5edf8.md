...
#### Reduce Pattern in LangGraph {#reducer_pattern}
...
In language each `node` is actually a python function that satisfies the following reducer interface:
...
#### Open a Webpage, Scrap data, Get Specfic Info and Send it via Email
...
### Multi-Agent Workflow in LangGraph
...
#### Define Two Nodes by Two Agents
...
- **Safety.** All operations restricted to `root_dir="sandbox"` directory.
...
- **Safety**. **High risk** - Can execute arbitrary code. Use with caution.
...
- **Returns.** Text summary of Wikipedia articles (first few paragraphs).

- **Safety.** **Read-only** - external API, safe for production.
