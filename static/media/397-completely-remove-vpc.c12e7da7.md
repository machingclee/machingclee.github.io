...
##### Unified approach to delete lambda functions of case 2

Assume that we destroy entire VPC (including its resources), at some point we will be blocked by dangling ENIs. Now we delete and update functions that are using those ENIs.

First we borrow a script from AWS official repository:

###### Step 1: Find all functions that are using the ENI

We create a file called: `findEniAssociations` (no extension here)
...
to get a list of functions using this ENI. In my case I get the following list of functions
...
Having obtained a list of functions using the ENI
We use the following python script to do the delete and update process:
...
Our strategy is to create a function with exactly the same function name, `sg_id` and `subnet_id`'s.

Being of the same `sg_id` and `subnet_id`'s will prompt AWS to reuse the same ENI. Now remove the VPC config will trigger the asynchronous operation of ENI removal (when deletion rules are meet, namely, no published version is using the ENI).
...
3. Randomly pick a role, a create a function with desired security group id and subnet ids:
