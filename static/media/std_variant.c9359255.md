...
depending on a variable `act` passed into this constructor. This `std::variant` serve as the same purpose of type union in typescript (note!! `union` is a special keyword in C++ and does not work in the same way as typescript).
