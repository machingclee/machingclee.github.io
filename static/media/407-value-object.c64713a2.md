...
```
Here we have simply changed the primitive type to a data class `Birthdate`, with the value object be defined by:
...
When we convert an entity class into a plain DTO object, there are two approches to instruct the kspKotlin for data transform:
...
We adopt the second approach as it makes no change to our codebase when we do the refactoring. 
...
has updated the `GenerateDTOProcessor` class to scan all the `@Embedded`-annotated fields, and then scan all `@Column`-annotated fields contained in it for the `toDTO()` code generation process
