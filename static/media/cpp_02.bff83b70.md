...
```
We can observe that the ***unreused*** (we assume this is the case) variable returned from `create_data()` does not trigger the copy constructor, for which the copy constructor is supposed to allocate new memory to store our temporary object. 
