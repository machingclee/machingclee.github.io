...
If we want to create a module by directory `blockchain/`, we need an ***entrypoint*** named `mod.rs`[^modexample] in that directory.


##### Export

In `blockchain/mod.rs`[^modexample] or `blockchain/*.rs`, we mark the target object that we want to export by `pub`.
...
[^modexample]:
    ...
    impl Block {
        ...
        pub fn print(&self) {
            ...
            println!("trasnactions: {:?}", self.transactions);
        }
    ...
    impl BlockChain {
        ...
        pub fn search_block(&self, search: BlockSearch) -> BlockSearchResult {
            ...
            return BlockSearchResult::FailOrEmptyBlocks;
        }
    }
    ```
