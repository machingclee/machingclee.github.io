...
id: blog066
tag: deep-learning
intro: We modify from DCGAN from <a href="/blog/GAN-and-DCGAN-in-Tensorflow"><i>this post</i></a> to experiment on WGAN by weight-clipping, then we try to experiment WGAN-GP using gradient penality.
...
$$
where we assume that $Z\sim p$, for some density $p\in L^1(\mathbb R^{\texttt{z_dim}})$ with $\texttt{z_dim}$ being the latent dimension which we need to search by experiment (rigorously the ***equality holds*** by a standard real analysis trick: prove it for first characteristic functions, second simple functions, finally use monotone convergence etc).
...
- (cont'd) As we will not use signmoid output any more, usually we call $f_w$ a ***critic*** which replaces the role played by the usual $\log D$ trick. Then we can modify our discriminator to output any tensor of shape $(\texttt{batch_size}, 1)$.
...
With exactly the same setup as in <a href="/blog/GAN-and-DCGAN-in-Tensorflow"><i>DCGAN</i></a>, we edit the following constants:
