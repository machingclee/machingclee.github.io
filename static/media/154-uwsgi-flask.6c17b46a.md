...
- Usually in python thinks become tricky if we want to access object defined in **arbitrary** diectory. 
- It is very often to get `ModuleNotFoundError` and require tricky step like appending desired directory path to `sys.path` that includes `src`.
- Alternatively we can simply define an environment variable `PYTHONPATH` to be the directory that is parent to our `src/`.
