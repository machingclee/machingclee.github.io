...
1. What execution plan is used? 
2. Am I missing an index? 
3. Or the created index is not used at all, etc)

We don't have Aspire from the dotnet world as a observability monitoring tool, but we have tight integration of datasource-micrometer with opentelemetry in spring boot:
...
Note that when `management.otlp.tracing.endpoint` (line-13 in [#logging_in_console]) is a non-empty string, we need to set up a non-empty OTLP endpoint like:
- Jaeger
- Zipkin
