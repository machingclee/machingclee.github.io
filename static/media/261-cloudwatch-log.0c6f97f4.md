---
title: "Download CloudWatch Logging Entirely"
date: 2024-05-09
id: blog0261
tag: aws
...
def main():
    ...
    if LOG_GROUP is None:
        print("please provide --log_group=? For example: //ecs/billie-chat-prod, ('/' will be resolved into local file system in git-bash)")
        should_end = True
    else:
        LOG_GROUP = args.log_group.replace("//", "/")

    if FROM_TIMESTAMP is None:
        print("please provide --from_timestamp=?, example: \"2024-05-09 12:00:00\" in your local time")
        should_end = True
    else:
    ...
    i = 0
    with open(f"{LOG_DIR}/{FILE_NAME}.txt", "a+", encoding="utf8") as file:
        for log_stream_name in log_stream_names:
            ...
            while started is False or response['nextForwardToken'] is not None:
                ...
                if len(data_) == 0:
                    print("no more data                                                                                            ")
                    break
                print(f"Loading Page {page}, {len(data_)} lines                              ", end="\r")
                for datum in data_:
...
please provide --log_group=? For example: //ecs/billie-chat-prod, ('/' will be resolved into local file system in git-bash)
please provide --from_timestamp=?, example: "2024-05-09 12:00:00" in your local time
```
...
python cloudwatch.py \
    ...
    --from_timestamp="2024-05-08 01:00:00" \
    -n=10
```
...
<a href="/assets/img/2024-05-09-22-33-22.png">![](/assets/img/2024-05-09-22-33-22.png)</a>
