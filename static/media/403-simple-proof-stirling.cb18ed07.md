...
- **Fact 4 (Walli's Formula).** Define $I_n=\int_0^{\pi/2}\sin^n x\,dx$, show that
  $$
...
On the other hand, we observe that by **Fact 2** we can obtain a decreasing sequence by multiplying $b_n/b_{n+1}=\frac{1}{e}\left(1+\frac{1}{n}\right)^n$ a factor $(1+\frac{1}{n})^{1/2}$:
...
$$
a_n:= \frac{b_n}{\sqrt{n}} = \frac{e^n n!}{n^{n+1/2}}.
$$
...
$$
1<\frac{a_n}{a_{n+1}}<e^{\frac{1}{4}\left(\frac{1}{n}-\frac{1}{n+1}\right)}.
$$

Thus the first inequality tells us $\{a_n\}$ converges to a nonnegative limit $\boxed{\alpha:=\lim_{n\to\infty} a_n}$. On the other hand, the second inequality tell us $\{a_ne^{-\frac{1}{4n}}\}$ increases to $\alpha$, so $\alpha >0$. By using the following form of Walli's formula (modified from **Fact 4**)
