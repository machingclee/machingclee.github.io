...
Note that for a routing to work, `exact: false` is very important.
...
export default () => {
  ...
  const match = useRouteMatch<{ matchedArticleTitle: string }>(`${navRoutes.BLOG}/:matchedArticleTitle`);
  const { matchedArticleTitle } = match?.params || { matchedArticleTitle: "" };
  
  const activeBlogArticleTitle = useSelector((state: TRootState) => state.app.blog.activeBlogArticleTitle);
  const selectedArticle = articles.find(md => md.title === activeBlogArticleTitle);

  ...

  useEffect(() => {
    dispatch(appActions.updateBlog({ activeBlogArticleTitle: matchedArticleTitle }));
  }, [matchedArticleTitle]);
