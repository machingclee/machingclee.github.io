---
title: "Swagger-UI for Spring Boot in SnapStarted Lambda and Basic Functionalities"
date: 2025-01-02
id: blog0357
...
The config in the current section aims at solving 2 problems:

1. API-Gateway config problem (specific to lambda function ***only***)

2. How to automate the "token-header configruation" once we are authenticated via `login-url` (`/auth/login` in our case).
...
##### Util functions for stages

Since our config will be dependent on whether we use api-gateway. In general we need to divide our resource paths by `local`, `dev`, `uat` and `prod`.
...
##### Inject custom javascript to automation the process of setting token after login
...
```
##### Configure servers (`baseURLs`) and api info to the swagger documentation
...
#### More on Encoding Setting in application.yml
