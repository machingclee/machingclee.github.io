...
- Once compilation is successfully done, a QR code will be presented in terminal for downloading the app directly.
...
- Our flag `--profile development` in `eas build` will use the config in `eas.json`
  ...
  ```
  Under this flag, the ***environment variable*** in ***development build*** is not determined at compile time, we can change the environment variables when we spawn a dev-client.
...
#### Prebuild
- A ***prebuild*** is actually the stuff that is generated in `eas build` on cloud if we have never `prebuild` the project.
...
- This workflow is nowadays called a ***bare workflow***.
...
- ***Problem: What if they havn't?*** 

- Then we will need to ***prebuild the project***, this will generate `ios/` and `android/` folders containing the native code of the project. We then follow the guidelines to modify the native code on our own.
